// Modified by Beau
// add some printf's statements to indicate what is going on
// you can turn off the statements by settting the macro DEBUG 0
// 
...
//set this one to 0 to turn off printing 
#define DEBUG 1

// for debugging statements
#ifdef DEBUG
    #define print(b); printf("%s\n",b);
#else
    #define print(b); printf("");
#endif
...
#ifdef DEBUG
            printf("read from instruction cache");
#endif
...
#ifdef DEBUG
            printf("rfo from snooping processor\n");
#endif
			if (dc_hit(d_set, tag, &hit_index)) {
				LRU_update(d_set, hit_index);
				d_line = &d_set->d_line[hit_index];
				switch (d_line->mesi) {
					...
					case S:
						d_line->mesi = I;
#ifdef DEBUG
                        printf("set mesi bit to invalid\n\n");
#endif
						break;
				}
			} // Nothing to do if miss
